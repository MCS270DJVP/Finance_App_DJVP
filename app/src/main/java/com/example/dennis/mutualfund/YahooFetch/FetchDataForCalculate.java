            ...
            *If that is the case, update new data*/
            ...
            if (!isSameDate) {
             ...
             /*update the current price of the time when the calculate button is pressed*/
                    ...
                    Calendar pastTime1 = mFund.getTime();
            /*check if the current time is after the time when historical prices were fetched by one day
            * or after the trading market is closed at 4pm of the same day
            *If that is the case, update new data*/
                    boolean isSameDate1 = currentTime.get(Calendar.DAY_OF_YEAR) == pastTime1.get(Calendar.DAY_OF_YEAR)
                            && currentTime.get(Calendar.YEAR) == pastTime1.get(Calendar.YEAR)
                            && (currentTime.get(Calendar.HOUR_OF_DAY) >= 16 && pastTime1.get(Calendar.HOUR_OF_DAY) >= 16
                            || (currentTime.get(Calendar.HOUR_OF_DAY) < 16 && pastTime1.get(Calendar.HOUR_OF_DAY) < 16));
                    if (mFund.getHistoricalPrices() == null || !isSameDate1) {
                        try {
